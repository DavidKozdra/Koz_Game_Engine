...
When to use:
- Only if you still need the current game-specific travel-event runtime.

How to use:
- Construct `new EventSystem(options)` and inject `CountdownTimer` and `eventEngine` where possible.

Current caveat:
- This still contains game-specific runtime knowledge and should eventually move out or be split further.
...
When to use:
- Only if your host data already looks close to the engine's grid/pathing format.
...
When to use:
- Only if you still need the current game-specific browser runtime for minigames.
